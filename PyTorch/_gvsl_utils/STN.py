...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow, mode="bilinear"):
        ...
        grid, flow = grid.to(src.device), flow.to(src.device)
        new_locs = grid + flow
...
class AffineTransformer(nn.Module):
    ...
    def forward(self, src, mat, mode="bilinear"):
        norm = torch.tensor(
            [[1, 1, 1, src.shape[2]], [1, 1, 1, src.shape[3]], [1, 1, 1, src.shape[4]]],
            dtype=torch.float,
        ).cuda()
        norm = norm[np.newaxis, :, :]

        norm = norm.to(src.device)
        mat = mat.to(src.device)

        mat_new = mat / norm
        grid = nnf.affine_grid(
            mat_new, [src.shape[0], 3, src.shape[2], src.shape[3], src.shape[4]]
        )
